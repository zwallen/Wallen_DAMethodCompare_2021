...
write.table(results_all, "../Script_Output/Dataset2_Output/Results_summary_table_10%.txt", row.names=F, quote=F, sep="\t")
...
write.table(rownames_to_column(data.frame(conc.df[ncol(conc.df):1,], check.names=F), "Methods"), "../Script_Output/Dataset2_Output/Pairwise_concordances_10%.txt", row.names=F, quote=F, sep="\t")
