### Calculate concordances when taking replication across datasets into account
...
write.table(rownames_to_column(data.frame(conc.df[ncol(conc.df):1,], check.names=F), "Methods"), "../Script_Output/Joint_Analyses_Output/Pairwise_concordances_10%.txt", row.names=F, quote=F, sep="\t")
